...
import com.example.correctword.databinding.FragmentWordChar8Binding

class WordChar8 : Fragment() {
    private lateinit var binding: FragmentWordChar8Binding
    private val dateModel: DateModel by activityViewModels()
    private var wordTrue = arrayOfNulls<String>(8)
    private var arrayWord = arrayOfNulls<String>(8)
    private var arrayTextView = arrayOfNulls<TextView>(8)
    private var arrayTextViewTrue = arrayOfNulls<TextView>(8)
    ...
    override fun onCreateView(
        ...
        binding = FragmentWordChar8Binding.inflate(inflater)
        return binding.root
    }
    ...
    companion object {
        fun newInstance() = WordChar8().apply { Bundle().apply {} }
    }
    ...
    private fun fillArray(arrayTextTrue: Array<TextView?>, letter: CharSequence) {
        ...
        if (winWord(arrayTextTrue) && counter == 7) {
            counterTrueWord++
    ...
    private fun wordToTextView() {
        ...
        Word().wordChar8[number].forEachIndexed { index, char ->
            arrayWord[index] = char.toString()
        }
        Word().wordChar8[number].forEachIndexed { index, char -> wordTrue[index] = char.toString() }
        arrayWord.shuffle()
