...
import com.example.correctword.databinding.FragmentWordChar4Binding

class WordChar4 : Fragment() {
    private lateinit var binding: FragmentWordChar4Binding
    private val dateModel: DateModel by activityViewModels()
    private var wordTrue = arrayOfNulls<String>(4)
    private var arrayWord = arrayOfNulls<String>(4)
    private var arrayTextView = arrayOfNulls<TextView>(4)
    private var arrayTextViewTrue = arrayOfNulls<TextView>(4)
    ...
    override fun onCreateView(
        ...
        binding = FragmentWordChar4Binding.inflate(inflater)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        arrayTextView = arrayOf(binding.textChar1,binding.textChar2,binding.textChar3,binding.textChar4)
        arrayTextViewTrue = arrayOf(binding.textCharTrue1,binding.textCharTrue2,binding.textCharTrue3,binding.textCharTrue4)
    ...
    companion object {
        fun newInstance() = WordChar4().apply { Bundle().apply {} }
    }
    ...
    private fun fillArray(arrayTextTrue: Array<TextView?>, letter: CharSequence){
        ...
        if(winWord(arrayTextTrue) && counter == 3) {
            counterTrueWord++
    ...
    private fun wordToTextView(){
        ...
        Word().wordChar4[number].forEachIndexed{index, char -> arrayWord[index] = char.toString() }
        Word().wordChar4[number].forEachIndexed{index, char -> wordTrue[index] = char.toString() }
        arrayWord.shuffle()
    ...
    private fun startCountDownTimer(timeMillis: Long){
        ...
        timer = object: CountDownTimer(timeMillis,1){
            override fun onTick(timeMili: Long) {
                binding.textTimer.text = "Залишилось: \n  " + (timeMili / 1000).toString() + " секунд"
            }
