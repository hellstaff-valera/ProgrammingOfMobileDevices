...
import com.example.correctword.databinding.FragmentWordChar7Binding

class WordChar7 : Fragment() {
    private lateinit var binding: FragmentWordChar7Binding
    private val dateModel: DateModel by activityViewModels()
    private var wordTrue = arrayOfNulls<String>(7)
    private var arrayWord = arrayOfNulls<String>(7)
    private var arrayTextView = arrayOfNulls<TextView>(7)
    private var arrayTextViewTrue = arrayOfNulls<TextView>(7)
    ...
    override fun onCreateView(
        ...
        binding = FragmentWordChar7Binding.inflate(inflater)
        return binding.root
    }
    ...
    companion object {
        fun newInstance() = WordChar7().apply { Bundle().apply {} }
    }
    ...
    private fun fillArray(arrayTextTrue: Array<TextView?>, letter: CharSequence) {
        ...
        if (winWord(arrayTextTrue) && counter == 6) {
            counterTrueWord++
    ...
    private fun wordToTextView() {
        ...
        Word().wordChar7[number].forEachIndexed { index, char ->
            arrayWord[index] = char.toString()
        }
        Word().wordChar7[number].forEachIndexed { index, char -> wordTrue[index] = char.toString() }
        arrayWord.shuffle()
