...
import com.example.correctword.databinding.FragmentWordChar5Binding

class WordChar5 : Fragment() {
    private lateinit var binding: FragmentWordChar5Binding
    private val dateModel: DateModel by activityViewModels()
    private var wordTrue = arrayOfNulls<String>(5)
    private var arrayWord = arrayOfNulls<String>(5)
    private var arrayTextView = arrayOfNulls<TextView>(5)
    private var arrayTextViewTrue = arrayOfNulls<TextView>(5)
    ...
    override fun onCreateView(
        ...
        binding = FragmentWordChar5Binding.inflate(inflater)
        return binding.root
    }
    ...
    companion object {
        fun newInstance() = WordChar5().apply { Bundle().apply {} }
    }
    ...
    private fun fillArray(arrayTextTrue: Array<TextView?>, letter: CharSequence) {
        ...
        if (winWord(arrayTextTrue) && counter == 4) {
            counterTrueWord++
    ...
    private fun wordToTextView() {
        ...
        Word().wordChar5[number].forEachIndexed { index, char ->
            arrayWord[index] = char.toString()
        }
        Word().wordChar5[number].forEachIndexed { index, char -> wordTrue[index] = char.toString() }
        arrayWord.shuffle()
